import Modal from '../models/Category.js';
...
const getAll = async (req, res) => {
  try {
    ...
    const data = await Modal.find({ deletedAt: null }).sort( { createdAt: -1 } ).populate('parent');
...
const getAllParents = async (req, res) => {
  try {
    //const data = await Admin.find().sort( { createdAt: -1 } );
    const data = await Modal.find({ parent: null }).sort( { createdAt: -1 } );
...
const getAllDeleted = async (req, res) => {
  ...
  try {
    ...
    const data = await Modal.find({ deletedAt: { $ne: null } }).sort( { deletedAt: -1 } ).populate('parent');
...
const getById = async (req, res) => {
  try {
    const data = await Modal.findById(req.params.id).populate('parent');
    console.log(data);
    if (!data) {
