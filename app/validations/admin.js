...
const loginRules = [
    check('email')
        .notEmpty().withMessage(loginMsg.email_required)
        .isEmail().withMessage(loginMsg.email_valid),
    check('password')
        .notEmpty().withMessage(loginMsg.password_required)
        .isLength({ min: 8 }).withMessage(loginMsg.password_min)
];

export { createRules, loginRules };
